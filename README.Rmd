...
```R
topic <- create_topic(name = "Test Topic")
...
```R
subscribe(topic, "email", "me@example.com")
subscribe(topic, "SMS", "1-111-555-1234")
```
...
By default, the message is sent to all platforms. This may not be ideal if multiple dissimilar endpoints are subscribed to the same topic (e.g., SMS and email). This can be resolved by maintaining separate Topics or, more easily, by sending different messages to each type of endpoint:
...
publish(topic = topic, message = message, subject = "Hello!")
```
